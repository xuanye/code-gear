...
class CodeGear {
  ...
  async generate(configUrl) {
    ...
    try {
      ...
      config.type = config.type || "mysql"; //adapter version  1.0.5 and lower
  ...
  prepareTemplates(config) {
    ...
    if (config.imports) {
      const importsFun = require(path.resolve(process.cwd(), config.imports));
  ...
  generateInOneFile(template, schema) {
    ...
    //Allow filtering of partial tables
    if (!filePartPath) {
      return;
    }
